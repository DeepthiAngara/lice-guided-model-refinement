"""Build prediction-level audit artifacts and verify stable workflow outputs.
...
PATTERN_PATH = PROJECT_ROOT / "predictions" / "oof" / "lice_pattern_match_trace.csv.gz"
PREDICTION_PATH = PROJECT_ROOT / "predictions" / "test" / "eight_configuration_test_predictions_wide.csv.gz"
METRIC_PATH = PROJECT_ROOT / "results" / "test_ablation" / "metrics_full_precision.csv"
MCNEMAR_PATH = PROJECT_ROOT / "results" / "test_ablation" / "mcnemar_results_full_precision.csv"
REFERENCE_PATH = (
...
def compare_result_summary(
    summary_name: str, filename: str, observed: pd.DataFrame
) -> None:
    expected = pd.read_csv(
        TABLE_DIR / filename, dtype=str, keep_default_na=False
    )
    ...
    record_check(
        f"{summary_name}_reproduction",
        summary_name.replace("_", " ").title(),
        same_values,
        detail,
...
test_audit_path = PROJECT_ROOT / "predictions" / "test" / "eight_configuration_test_predictions_long.csv.gz"
...
training_audit_path = PROJECT_ROOT / "predictions" / "oof" / "training_lice_assignment_audit.csv.gz"
...
# Test-ablation performance summary
ablation_performance_summary = pd.DataFrame(
    [
        {
...
# False-negative and false-positive trade-off summary
tradeoff_summary_rows = []
for model in MODEL_ORDER:
...
tradeoff_summary = pd.DataFrame(tradeoff_summary_rows)

# Incremental interaction-effect summary
interaction_effect_rows = []
for label, weighting_model, combined_model in [
    ...
    interaction_effect_rows.append(
        {
...
interaction_effect_summary = pd.DataFrame(interaction_effect_rows)

# Criterion-specific operating-point summary
operating_criteria_rows = []
for criterion, column, direction, formatter in [
    ...
    operating_criteria_rows.append(
        {
            "Criterion": criterion,
...
operating_criteria_summary = pd.DataFrame(operating_criteria_rows)

# Positive-case McNemar comparison summary
positive_mcnemar = computed_mcnemar.loc[
    computed_mcnemar["Scope"].eq("Actual positive cases only")
].copy()
positive_case_mcnemar_summary = pd.DataFrame(
    {
...
# Selected operating-point and baseline-change summaries
selected_models = ["Baseline", "M3-Balanced", "M3-High"]
selected_operating_points_summary = pd.DataFrame(
    [
        {
...
baseline_change_summary = pd.DataFrame(baseline_change_rows)

# Cross-study context summary
cross_study_rows = [
    {
...
cross_study_summary = pd.DataFrame(cross_study_rows)

for summary_name, filename, summary_frame in [
    ("test_ablation_performance", "test_ablation_performance.csv", ablation_performance_summary),
    ("false_negative_false_positive_tradeoff", "false_negative_false_positive_tradeoff.csv", tradeoff_summary),
    ("incremental_interaction_effect", "incremental_interaction_effect.csv", interaction_effect_summary),
    ("criterion_specific_operating_points", "criterion_specific_operating_points.csv", operating_criteria_summary),
    ("mcnemar_positive_case_comparison", "mcnemar_positive_case_comparison.csv", positive_case_mcnemar_summary),
    ("operating_point_summary", "operating_point_summary.csv", selected_operating_points_summary),
    ("metric_change_from_baseline", "metric_change_from_baseline.csv", baseline_change_summary),
    ("cross_study_context", "cross_study_context.csv", cross_study_summary),
]:
    compare_result_summary(summary_name, filename, summary_frame)
...
manifest = {
    "release_tag": "v1.3.0",
    "purpose": (
        "Prediction-level audit and independent reproduction of workflow-named "
        "result summaries from released case-level predictions."
    ),
    "test_set_semantics": {
        "outer_split": "test",
    ...
    "tolerances": {
        ...
        "formatted_result_summary_comparison": "exact after declared rounding",
    },
    "lice_patterns": {
        "FN_P1": "GenHlth <= 2",
        "FN_P2": "HighBP <= 0",
...
print("Prediction-level audit and stable-output verification completed successfully.")
